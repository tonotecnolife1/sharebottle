...
MVP では **キャスト選択型ログイン** を採用しています（メール/パスワード不要）。

- `/auth/login` で 5 名のデモキャストから選択 → httpOnly cookie `nightos.mock-cast-id` を発行
- `middleware.ts` が cookie をチェックし、未ログインは `/auth/login` へリダイレクト
- `lib/nightos/auth.ts` の `getCurrentCast()` が cookie を読んで cast レコードを解決
- Supabase Auth への移行用の `auth_user_id` カラムは既に schema にあります（未使用）

将来的には Supabase Auth に差し替え予定ですが、MVP 検証中は選択型で十分です。
