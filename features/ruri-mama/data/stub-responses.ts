...
export function generateStubReply(opts: {
  ...
  if (intent === "follow") {
    ...
    return `**【アドバイス】**
${name}さんへの${purpose}の連絡ね。${tone}送るのが正解よ。${moodComment}

**【文面例】**
「${surname}さん、先日はありがとうございました。${topicLine}${bottleLine}」
...
具体的な話題を一つだけ織り込むのがポイントよ。「また会いたい」を重くならずに伝えるの。${
  ...
  if (intent === "serving") {
    ...
    const exampleText =
      situation === "指名につなげたい"
        ? lastTopic
          ? `「${surname}さん、さっきの${lastTopic}の話、気になって私も調べちゃった。次いらした時に続き聞かせてくださいね。」`
          : `「${surname}さん、今日お話できて嬉しかったです。次は私の勝手なおすすめも聞いてもらいたいし、またぜひ。」`
        : situation === "ボトル提案したい"
          ? `「${surname}さん、最近入ったボトルで面白いのがあるんです。次いらした時に一杯試してみませんか？」`
          : situation === "機嫌悪い時"
      ...
      cause === "指名化できない"
        ...
        : cause === "常連離れ"
          ...
          : cause === "新規来ない"
            ...
            : "まずフォロー頻度を整えるのが先ね。それで半分解決するから。";

    const exampleText = `「${surname}さん、お元気ですか？最近こんな面白いお客様にお会いしたんです。きっと気が合うと思うので、今度ご一緒しません？」`;
...
${
  lastTopic
    ? `「${surname}さん、${lastTopic}のこと、その後どうなりましたか？ふと気になって連絡してしまいました。」`
    : `「${surname}さん、お元気ですか？またお顔を見せてくださいね。」`
}
